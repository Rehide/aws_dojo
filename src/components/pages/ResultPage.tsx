        ...
        {weakDomains.length > 0 && (
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
            <p className="text-sm font-medium text-amber-800">
              ⚠ 弱点:{" "}
              {weakDomains.map((s) => `Domain ${s.domain}`).join("、")}{" "}
              の正答率が最も低くなっています
            </p>
          </div>
        )}
                        ...
                        <div className="space-y-1.5">
                          {a.displayChoices.map((choice) => {
                            ...
                            const isSelectedWrong =
                              choice.id === a.selectedChoiceId && a.isCorrect === false;
                            ...
                            const icon = isCorrectChoice ? (
                              ...
                              <span className="mt-0.5 shrink-0 text-xs font-bold text-red-600">✗</span>
                            ) : (
